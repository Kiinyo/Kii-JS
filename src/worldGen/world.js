Kii.World = function (template) {
    ...
    this.lookUp = function (x, y) { //This function returns the region, chunk, and tile of a given coordinate
      ...
      if (this._width > -1) {
        if (x < 0 || x >= this._height * cs * rs) {inBounds = false}
      }

      if (this._height > -1) {
        if (y < 0 || y >= this._width * cs * rs) {inBounds = false}
      }
    ...
    //Creating the currently loaded Area
    ...
    //Handling movement inside an area
    this.isInsideArea = function (coords) { //Simply making sure a coordinate is inside the Area
      return (this.ActiveArea.Tiles.length > coords[1] && 
              this.ActiveArea.Tiles[0].length > coords[0] &&
              coords[0] >= 0 && coords[1] >= 0)
    }
    this.checkTile = function (coord) { //Checking if it's possible to move to a tile and returning the tile if true
      coords = Kii.Util.fPIA(coord, this.ActiveArea.Tiles)
      
      if (this.isInsideArea(coords)) {
        return this.ActiveArea.Tiles[coords[1]][coords[0]]
      } else {
        return null
      }
    }
    this.leaveTile = function (entity) { //Exiting a tile
      let coords = Kii.Util.fPIA(entity, this.ActiveArea.Tiles)
      this.ActiveArea.Tiles[coords[1]][coords[0]].exit(entity)
    }
    this.enterTile = function (entity) { //Entering a tile
      let coords = Kii.Util.fPIA(entity, this.ActiveArea.Tiles)
    ...
    //World Updates
    this.updateLocal = function (time, args) {
      //Placeholder
    }
    this.updateGlobal = function (time, args) {
      //Placeholder
    }
  }